...
export default function EditProfilePage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const updatedUser = await updateMe({ username }); // ✅ передаємо об’єкт
      setUser(updatedUser); // ✅ оновлюємо глобальний стор
      router.push("/profile");
    } catch {
      setError("Failed to update profile"); // ✅ показуємо помилку користувачу
    }
  };
